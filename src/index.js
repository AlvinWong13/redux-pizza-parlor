...
const checkout = (state={totalPrice: 0, checkoutList:[]}, action) => {
  switch (action.type) {
    case 'ADD_PIZZA':
      ...
      return { totalPrice:totalPrice += (action.payload.price), checkoutList:[...state.checkoutList, action.payload] };
    case 'REMOVE_PIZZA':
      let newCheckout = state.checkoutList.filter(
        stat => stat.id != action.payload.id
      );
      return {totalPrice: state.totalPrice - action.payload.price, checkoutList:newCheckout};
    default:
      return state;
  }
}
